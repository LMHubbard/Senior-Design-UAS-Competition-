...
ALIGN_ALTITUDE = 2.5 # altitude to align vehicle to marker at
...
def align_vehicle_relative_to_marker(vehicle, all_distances):
    ...
    new_location = LocationGlobal(new_latitude, new_longitude, vehicle.location.global_frame.lon,
        home_altitude + ALIGN_ALTITUDE)
...
def get_image_dimensions_meters(dimensions, camera_matrix):
    """
    ...
    GSD_h = (ALTITUDE * sensor_height) / (focal_length * frame_height)
    GSD_w = (ALTITUDE * sensor_width) / (focal_length * frame_width)

    return (GSD_w * frame_width, GSD_h * frame_height)
        ...
        if not queue.empty():
            ...
            if run_initial_search is True:
                ...
                if has_path_been_generated is False:
                    ...
                    frame_size_meters[0],frame_size_meters[1] = get_image_dimensions_meters(frame.shape, camera_matrix)

                    write_arduino_serial(90, 90, 90, 0, 0)
                    time.sleep(2)
                    write_arduino_serial(90, 90, 90, 0, 0)
                    arm_drone(vehicle)  # arm the vehicle
                    search_waypoints = run_path_generation(vehicle, frame_size_meters[0], frame_size_meters[1]) # generate the path
                    print("success")
