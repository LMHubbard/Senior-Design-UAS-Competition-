...
ALIGN_VEHICLE_TO_MARKER_ALT = 1.75 # meters to align vehicle at eventually
...
xy_error_threshold_stage1 = 1  # the threshold under which to begin dropping altitude while aligning
xy_error_threshold_stage2 = 0.15  # the threshold under which to stop aligning vehicle, and begin engaging
...
def align_vehicle_relative_to_marker(vehicle, all_distances, gain):
    ...
    new_latitude = current_location.lat + ((y_movement*gain) / 111111)  # 1 degree latitude is approximately 111111 meters
    new_longitude = current_location.lon + ((x_movement*gain) / (111111 * math.cos(math.radians(current_location.lat))))  # Longitude varies with latitude
        ...
        if not queue.empty():
            ...
            if run_initial_search is True:
                ...
                if has_path_been_generated is False:
                    ...
                    camera_frame_width, camera_frame_height = get_image_dimensions_meters(frame.shape, camera_matrix,ALTITUDE)

                    write_arduino_serial(90, 90, 90, 0, 0)
                    time.sleep(2)
                    write_arduino_serial(90, 90, 90, 0, 0)
                    arm_drone(vehicle)  # arm the vehicle
                    search_waypoints, point1, point2 = run_path_generation(vehicle, camera_frame_width, camera_frame_height) # generate the path
                    print("success")

                    has_path_been_generated = True  # dont run path gen again
                    time.sleep(3)
                    takeoff_drone(vehicle, ALTITUDE)  # take the vehicle off
                    heading_deg = calculate_heading(point1, point2)
                    
                    
                    
                    #vehicle.simple_goto(vehicle.location.global_frame.lat, vehicle.location.global_frame.lon,
                     #                   vehicle.location.global_frame.alt, yaw=heading_deg)


                if marker_to_engage == -1:
                    if search_waypoints is not None:
                        current_waypoint = search_waypoints[search_waypoint_index]
                        if go_away_bad_guy is True:
            ...
            if align_vehicle_to_marker is True:
                ...
                if distance_x is not None and distance_y is not None and distance_z is not None:
                    last_known_marker_location = target_location
                    total_xy_error_average, target_location = align_vehicle_relative_to_marker(vehicle, all_distances,0.85)
                    ...
                    if align_xy_stage_1_complete is True:  # if the vehicle is close enough to engage
                        ...
                        new_loc = LocationGlobal(target_location.lat, target_location.lon, target_altitude)
                    vehicle.simple_goto(new_loc)
                elif last_known_marker_location is not None:
                    # go to last reported marker location
                    pass
